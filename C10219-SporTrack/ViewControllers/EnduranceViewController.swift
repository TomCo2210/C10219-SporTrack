...
class EnduranceViewController: UIViewController {
    ...
    private func saveEndurance() {
        ...
        do {
            ...
            try db.collection(K.FStore.EnduranceCollection).document(Auth.auth().currentUser!.uid).collection(dateFormatter.string(from: Date())).document().setData(from: newEndurance)
        } catch {
...
//extension EnduranceViewController: SegueHandlerType {
